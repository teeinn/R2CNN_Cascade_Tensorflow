...
class DetectionNetwork(object):
    ...
    def postprocess_cascade(self, rois, bbox_ppred, scope):
        ...
        with tf.name_scope('postprocess_cascade_r_{}'.format(scope)):
            rois = tf.stop_gradient(rois)
            bbox_ppred = tf.stop_gradient(bbox_ppred)
            ...
            with tf.variable_scope('rotation_branch'):
                with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                    ...
                    if not scope == 'stage3':
                        ...
                        bbox_pred_r_ins = tf.reshape(bbox_pred_r, [-1, cfgs.CLASS_NUM + 1, 5])

                        # only keep a box which score is the bigest
                        keep_abox = tf.argmax(cls_score_r, axis=1)
                        keep_inds = tf.reshape(tf.transpose(tf.stack([tf.cumsum(tf.ones_like(keep_abox)) - 1, keep_abox])), [-1, 2])
                        bbox_pred_fliter_r = tf.reshape(tf.gather_nd(bbox_pred_r_ins, keep_inds), [-1, 5])

                        return bbox_pred_h, cls_score_h, bbox_pred_fliter_r, bbox_pred_r, cls_score_r
                    else:
                        cls_score_r = tf.reshape(cls_score_r, [-1, cfgs.CLASS_NUM + 1])
                        bbox_pred_r = tf.reshape(bbox_pred_r, [-1, 5 * (cfgs.CLASS_NUM + 1)])
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
                   ...
                   bbox_pred_r, bbox_targets_r, cls_score_r, labels, stage3_labels,
            ...
            with tf.variable_scope('FastRCNN_loss'):
                if not cfgs.FAST_RCNN_MINIBATCH_SIZE == -1:
                    ...
                    bbox_loss_r = losses.smooth_l1_loss_rcnn_r(bbox_pred=bbox_pred_r,
                                                               ...
                                                               num_classes=cfgs.CLASS_NUM + 1,
                                                               sigma=cfgs.FASTRCNN_SIGMA)
                    stage2_bbox_loss = losses.smooth_l1_loss_rcnn_r(bbox_pred=stage2_bbox_pred,
                                                                  bbox_targets=stage2_bbox_targets,
                                                                  label=stage2_labels,
                                                                  num_classes=cfgs.CLASS_NUM + 1,
                                                                  sigma=cfgs.FASTRCNN_SIGMA)
                    stage1_bbox_loss = losses.smooth_l1_loss_rcnn_r(bbox_pred=stage1_bbox_pred,
                    ...
                    bbox_loss_r = bbox_loss_r + stage2_bbox_loss + stage1_bbox_loss
                    cls_loss_r = cls_loss_r + stage2_cls_loss + stage1_cls_loss
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch):
        ...
        if self.is_training:
            ...
            with tf.control_dependencies([rpn_labels]):
                with tf.variable_scope('sample_RCNN_minibatch_stage1'):
                    stage1_rois, stage1_labels, bbox_targets_h, stage1_bbox_targets_r = \
                    ...
                    stage1_bbox_targets_r = tf.reshape(stage1_bbox_targets_r, [-1, 5*(cfgs.CLASS_NUM+1)])
        ...
        bbox_pred_h, cls_score_h, stage1_bbox_pred_fliter_r, stage1_bbox_pred_r, stage1_cls_score_r = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                                 rois=stage1_rois,
        ...
        stage1_cls_prob_r = slim.softmax(stage1_cls_score_r, 'cls_prob_r')
        stage1_cls_category_r = tf.argmax(stage1_cls_prob_r, axis=1)

        # ----------------------------------------------add smry-------------------------------------------------------
        if self.is_training:
            cls_category_h = tf.argmax(cls_prob_h, axis=1)
            fast_acc_h = tf.reduce_mean(tf.to_float(tf.equal(cls_category_h, tf.to_int64(stage1_labels))))
            tf.summary.scalar('ACC/fast_acc_h', fast_acc_h)

            fast_acc_r = tf.reduce_mean(tf.to_float(tf.equal(stage1_cls_category_r, tf.to_int64(stage1_labels))))
        ...
        stage1_bbox = self.postprocess_cascade(rois=stage1_rois,
                                               bbox_ppred=stage1_bbox_pred_fliter_r,
                                               scope='stage1')
        if self.is_training:
            with tf.control_dependencies([stage1_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage2'):
                    ...
                    tf.py_func(proposal_target_layer,
                               [stage1_bbox, gtboxes_r_batch, cfgs.FAST_RCNN_IOU_POSITIVE_THRESHOLD[1]],
        ...
        _, _, stage2_bbox_pred_fliter, stage2_bbox_pred, stage2_cls_score = self.build_fastrcnn(
            ...
            with tf.control_dependencies([stage2_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage3'):
                    stage3_rois, stage3_labels, stage3_bbox_targets = \
                        tf.py_func(proposal_target_layer,
                                   [stage2_rois, gtboxes_r_batch, cfgs.FAST_RCNN_IOU_POSITIVE_THRESHOLD[2]],
                                   [tf.float32, tf.float32, tf.float32])
        ...
        _, _, stage3_bbox_pred, stage3_cls_score = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                 rois=stage3_rois,
                                                                 img_shape=img_shape,
                                                                 scope='stage3')
        # bbox_pred shape: [-1, 4*(cls_num+1)].
        ...
        if self.is_training:
            ...
            tf.summary.scalar('ACC/stage3_fast_acc', fast_acc)
        ...
        if not self.is_training:
            ...
            final_boxes_r, final_scores_r, final_category_r = self.postprocess_fastrcnn_r(rois=stage3_rois,
            ...
            loss_dict = self.build_loss(rpn_box_pred=rpn_box_pred,
                                        ...
                                        bbox_pred_r=stage3_bbox_pred,
                                        bbox_targets_r=stage3_bbox_targets,
                                        stage2_bbox_pred=stage2_bbox_pred,
                                        stage2_bbox_targets=stage2_bbox_targets,
                                        stage1_bbox_pred=stage1_bbox_pred_r,
                                        stage1_bbox_targets=stage1_bbox_targets_r,
                                        cls_score_r=stage3_cls_score,
                                        labels=stage1_labels,
                                        stage3_labels=stage3_labels,
                                        stage2_cls_score=stage2_cls_score,
                                        stage2_labels=stage2_labels,
                                        stage1_cls_score=stage1_cls_score_r,
