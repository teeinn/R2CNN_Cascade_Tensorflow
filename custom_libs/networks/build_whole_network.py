...
class DetectionNetwork(object):
    ...
    def postprocess_cascade(self, rois, bbox_ppred, scope):
        ...
        with tf.name_scope('postprocess_cascade_h_{}'.format(scope)):
            rois = tf.stop_gradient(rois)
            bbox_ppred = tf.stop_gradient(bbox_ppred)

            # decode boxes
            decoded_boxes = encode_and_decode.decode_boxes(encode_boxes=bbox_ppred,
                                                           reference_boxes=rois,
                                                           scale_factors=cfgs.ROI_SCALE_FACTORS)
    ...
    def build_fastrcnn(self, feature_to_cropped, rois, img_shape, scope):
        ...
        with tf.variable_scope('Fast-RCNN_{}'.format(scope)):
            ...
            with tf.variable_scope('horizen_branch'):
                with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                    ...
                    if not scope == 'stage3':
                        ...
                        bbox_pred_h_ins = tf.reshape(bbox_pred_h, [-1, cfgs.CLASS_NUM + 1, 4])

                        # only keep a box which score is the bigest
                        keep_abox = tf.argmax(cls_score_h, axis=1)
                        keep_inds = tf.reshape(
                            tf.transpose(tf.stack([tf.cumsum(tf.ones_like(keep_abox)) - 1, keep_abox])), [-1, 2])
                        bbox_pred_fliter_h = tf.reshape(tf.gather_nd(bbox_pred_h_ins, keep_inds), [-1, 4])
            ...
            with tf.variable_scope('rotation_branch'):
                ...
                if not scope == 'stage3':
                    return bbox_pred_fliter_h, bbox_pred_h, cls_score_h, bbox_pred_r, cls_score_r
                else:
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
                   bbox_pred_h, bbox_targets_h, cls_score_h,
                   bbox_pred_r, bbox_targets_r, cls_score_r,
                   labels, stage3_labels,
                   stage2_bbox_pred, stage2_bbox_targets,  stage1_bbox_pred, stage1_bbox_targets,
                   stage2_cls_score, stage2_labels, stage1_cls_score, stage1_labels):
        '''
        ...
        with tf.variable_scope('build_loss') as sc:
            ...
            with tf.variable_scope('FastRCNN_loss'):
                if not cfgs.FAST_RCNN_MINIBATCH_SIZE == -1:
                    ...
                    stage2_bbox_loss = losses.smooth_l1_loss_rcnn_h(bbox_pred=stage2_bbox_pred,
                                                                  bbox_targets=stage2_bbox_targets,
                                                                  label=stage2_labels,
                                                                  num_classes=cfgs.CLASS_NUM + 1,
                                                                  sigma=cfgs.FASTRCNN_SIGMA)
                    stage1_bbox_loss = losses.smooth_l1_loss_rcnn_h(bbox_pred=stage1_bbox_pred,
                    ...
                    cls_loss_h = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                        logits=cls_score_h,
                        labels=stage3_labels))  # beacause already sample before
                    ...
                    bbox_loss_h = bbox_loss_h + stage2_bbox_loss + stage1_bbox_loss
                    cls_loss_h = cls_loss_h + stage2_cls_loss + stage1_cls_loss
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch):
        ...
        if self.is_training:
            ...
            with tf.control_dependencies([rpn_labels]):
                with tf.variable_scope('sample_RCNN_minibatch_stage1'):
                    stage1_rois, stage1_labels, stage1_bbox_targets_h, bbox_targets_r = \
                    tf.py_func(proposal_target_layer,
                               [rois, gtboxes_h_batch, gtboxes_r_batch, cfgs.FAST_RCNN_IOU_POSITIVE_THRESHOLD[0]],
                    ...
                    stage1_bbox_targets = tf.reshape(stage1_bbox_targets_h, [-1, 4*(cfgs.CLASS_NUM+1)])
        ...
        stage1_bbox_pred_fliter, stage1_bbox_pred, stage1_cls_score, bbox_pred_r, cls_score_r = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                                 rois=stage1_rois,
                                                                                 img_shape=img_shape, scope='stage1')
        ...
        stage1_cls_prob_h = slim.softmax(stage1_cls_score, 'cls_prob_h')
        stage1_cls_category_h = tf.argmax(stage1_cls_prob_h, axis=1)
        cls_prob_r = slim.softmax(cls_score_r, 'cls_prob_r')

        # ----------------------------------------------add smry-------------------------------------------------------
        if self.is_training:
            stage1_fast_acc_h = tf.reduce_mean(tf.to_float(tf.equal(stage1_cls_category_h, tf.to_int64(stage1_labels))))
            # tf.summary.scalar('ACC/fast_acc_h', stage1_fast_acc_h)

            cls_category_r = tf.argmax(cls_prob_r, axis=1)
            fast_acc_r = tf.reduce_mean(tf.to_float(tf.equal(cls_category_r, tf.to_int64(stage1_labels))))
        ...
        if self.is_training:
            with tf.control_dependencies([stage1_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage2'):
                    stage2_rois, stage2_labels, stage2_bbox_targets, _ = \
                    tf.py_func(proposal_target_layer,
                               [stage1_bbox, gtboxes_h_batch, gtboxes_r_batch, cfgs.FAST_RCNN_IOU_POSITIVE_THRESHOLD[1]],
                               [tf.float32, tf.float32, tf.float32, tf.float32])
        ...
        stage2_bbox_pred_fliter, stage2_bbox_pred, stage2_cls_score, _, _ = self.build_fastrcnn(
        ...
        if self.is_training:
            with tf.control_dependencies([stage2_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage3'):
                    stage3_rois, stage3_labels, stage3_bbox_targets, _ = \
                        tf.py_func(proposal_target_layer,
                                   [stage2_rois, gtboxes_h_batch, gtboxes_r_batch, cfgs.FAST_RCNN_IOU_POSITIVE_THRESHOLD[2]],
                                   [tf.float32, tf.float32, tf.float32, tf.float32])
        ...
        stage3_bbox_pred, stage3_cls_score, _, _ = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                 rois=stage3_rois,
                                                                 img_shape=img_shape,
                                                                 scope='stage3')
        # bbox_pred shape: [-1, 4*(cls_num+1)].
        ...
        if self.is_training:
            ...
            tf.summary.scalar('ACC/fast_acc_h', fast_acc)
        ...
        if not self.is_training:
            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                                ...
                                reuse=True):
                    _, _, final_scores_stage2, _, _ = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                    rois=stage3_rois,
                                                                    img_shape=img_shape,
                                                                    scope='stage2')
                    final_scores_stage2 = slim.softmax(final_scores_stage2, 'final_scores_stage2')

                    _, _, final_scores_stage1, _, _ = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                    rois=stage3_rois,
                                                                    img_shape=img_shape,
                                                                    scope='stage1')
                    ...
                    final_boxes_h, final_scores_h, final_category_h = self.postprocess_fastrcnn_h(rois=stage3_rois,
                                                                                                  bbox_ppred=stage3_bbox_pred,
                                                                                                  scores=cls_prob,
                                                                                                  img_shape=img_shape,
                                                                                                  scope='stage3')

            final_boxes_r, final_scores_r, final_category_r = self.postprocess_fastrcnn_r(rois=stage1_rois,
            ...
            loss_dict = self.build_loss(rpn_box_pred=rpn_box_pred,
                                        ...
                                        bbox_pred_h=stage3_bbox_pred,
                                        bbox_targets_h=stage3_bbox_targets,
